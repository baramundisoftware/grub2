...
handle_image (void *data, grub_efi_uint32_t datasize)
{
	  ...
	  efi_status = efi_call_3 (b->allocate_pool, GRUB_EFI_LOADER_DATA, buffer_size, (void**)&buffer);
	  ...
	  {
		  grub_error (GRUB_ERR_BAD_OS, "could not start image: efi status %d", efi_status);
		  goto error_exit;
	  }
...
grub_err_t pxechainloader(char *filename, const bara_efi_ip_address_t* const serverip, const grub_efi_boolean_t always_use_secureboot_chainload)
{
	...
	{
		grub_error(GRUB_ERR_BAD_OS, N_("Cannot get filesize of file %s: statuscode %d"), filename, status);
		goto fail;
	}
	...
	address = 0;
	status = efi_call_4(grub_efi_system_table->boot_services->allocate_pages, GRUB_EFI_ALLOCATE_ANY_PAGES, GRUB_EFI_LOADER_CODE, pages, &address);
	...
	if (status != GRUB_EFI_SUCCESS)
	{
		grub_error(GRUB_ERR_BAD_OS, N_("pxechainloader cannot read file %s: statuscode %d"), filename, status);
		goto fail;
	}
	...
	status = efi_call_6(grub_efi_system_table->boot_services->load_image, 0, grub_efi_image_handle, file_path, boot_image, fsize, &image_handle);

	if (address)
	{
		efi_call_2(grub_efi_system_table->boot_services->free_pages, address, pages);
		address = 0;
	}
