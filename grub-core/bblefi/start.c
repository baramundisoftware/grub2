...
grub_err_t showMenu(bara_efi_ip_address_t* menu_server_ip,
					bara_efi_ip_address_t* tftp_server_ip,
...
grub_err_t processChosenMenuEntry(const bara_efi_ip_address_t* server_ip,
								  grub_efi_mac_t* client_mac,
					...
					int noOfServers)
{
	...
	else
	{
		// set defaults
		*chosenEntry = NULL;
		grub_memset(menu_server_ip, 0, sizeof(bara_efi_ip_address_t));
		*defaultchosen = 0;
...
grub_err_t processChosenMenuEntry(const bara_efi_ip_address_t* boot_server_ip,
								  grub_efi_mac_t* client_mac,
								  const BootLoaderMenuEntryT* const chosenEntry,
								  const int defaultchosen,
								  const grub_efi_boolean_t dont_set_pathprefix,
...
grub_err_t StartProcessing(void)
{
	...
	bara_efi_ip_address_t tftp_server_ip;
	grub_memset(&tftp_server_ip, 0, sizeof(bara_efi_ip_address_t));
	...
	int defaultchosen = 0;
	bara_efi_ip_address_t menu_server_ip;
	grub_memset(&menu_server_ip, 0, sizeof(bara_efi_ip_address_t));
	...
	{
		bara_efi_ip_address_t boot_server_ip = (FALSE != boot_from_menu_server) ? menu_server_ip : tftp_server_ip;
