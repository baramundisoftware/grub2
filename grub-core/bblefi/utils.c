...
grub_efi_status_t tftpGetFileSize(const bara_efi_ip_address_t* server_ip, const char* const filename, grub_uint64_t* filesize)
{
	...
	*filesize = 0;
	...
	status = efi_call_10(((bara_efi_pxe_base_code_mtftp)pxe->mtftp), pxe, EFI_PXE_BASE_CODE_TFTP_GET_FILE_SIZE, NULL, overwrite,
							filesize, &blocksize, server_ip, filename, NULL, nobuffer);

	return status;
}

grub_efi_status_t tftpReadFile(const bara_efi_ip_address_t* server_ip, const char* const filename, void* buffer, grub_uint64_t buffersize)
{
	grub_efi_status_t status = GRUB_EFI_SUCCESS;
	grub_efi_pxe_t* pxe = getEfiPxeBaseCodeProtocol();
	...
	status = efi_call_10(((bara_efi_pxe_base_code_mtftp)pxe->mtftp), pxe, EFI_PXE_BASE_CODE_TFTP_READ_FILE, buffer, overwrite,
							&buffersize, &blocksize, server_ip, filename, NULL, nobuffer);

	return status;
}

int dnsRequestServerIp(const char* const newServerIp, bara_efi_ip_address_t* ipaddr)
{
	grub_memset(ipaddr, 0, sizeof(bara_efi_ip_address_t));
	ipaddr->ipv4 = 0;
...
grub_err_t changeDhcpAckParms(const char* new_bootfilename, const bara_efi_ip_address_t* const server_ip)
{
	...
	else
	{
		...
		else
		{
			...
			if(pxe->mode->dhcp_ack_received)
			{
				...
				if(0 != server_ip->ipv4)
				{
					grub_memcpy(&dhcp_ack_packet->server_ip, &server_ip->ipv4, 4);
...
// Extract IP-Address to a readable/printable string
//grub_err_t convertIpToString(bara_efi_ip_address_t* src, char* ipAsString)
//{
//	grub_errno = GRUB_ERR_NONE;
//
//	if(src && ipAsString)
//	{
//		grub_memset(ipAsString, 0, 16);
//
//		grub_snprintf(ipAsString, 16,
//						"%d.%d.%d.%d",
//						(src->ipv4 & 0xFF),
//						((src->ipv4 >> 8) & 0xFF),
//						((src->ipv4 >> 16) & 0xFF),
...
grub_err_t getDhcpServerIp(bara_efi_ip_address_t* server_ip)
{
	grub_errno = GRUB_ERR_NONE;
	...
	{
		grub_memset(server_ip, 0, sizeof(bara_efi_ip_address_t));
		grub_efi_pxe_t *pxe = getEfiPxeBaseCodeProtocol();
		...
		if(NULL != pxe &&  NULL != pxe->mode)
		{
			if(pxe->mode->pxe_reply_received)
			{
				...
				grub_memcpy(&server_ip->ipv4, &pxe_reply->server_ip, 4);
			}
...
grub_err_t printEfiIpv4Address(const bara_efi_ip_address_t* const ip_address)
{
	grub_errno = GRUB_ERR_NONE;

	if(ip_address)
	{
		grub_printf("%d.%d.%d.%d", (ip_address->ipv4 & 0xFF), ((ip_address->ipv4 >> 8) & 0xFF), ((ip_address->ipv4 >> 16) & 0xFF), ((ip_address->ipv4 >> 24) & 0xFF));
	}
	else
	{
		grub_error(GRUB_ERR_BAD_ARGUMENT, "invalid argument (null)!");
...
int parseDhcpDiscoverOptions(int (*option_handler)(const grub_uint8_t, const grub_uint8_t, const grub_uint8_t*, void*), void* outdata)
{
	...
	if(pxe)
	{
		...
		ret = 0;
		unsigned int pos;
		for(pos = 0; pos < max_packet_size - options_section && pxe_mode->dhcp_discover[options_section + pos] != option_end; pos++)
		{
			// print the option number
			const grub_uint8_t option_number = pxe_mode->dhcp_discover[options_section + pos];

			// read the length of the option data
			const grub_uint8_t option_data_length = (grub_uint8_t)pxe_mode->dhcp_discover[options_section + (++pos)];

			// if the option handler returns 0 then we found the option
			if(0 == option_handler(option_number, option_data_length, &pxe_mode->dhcp_discover[options_section + pos + 1], outdata))
			{
				ret++;
				break;
			}
...
int grub_quit()
{
    ...
    efi_call_4 (grub_efi_system_table->boot_services->exit,
                grub_efi_image_handle, GRUB_EFI_NOT_STARTED, 0, 0);
    for (;;);
    return 0;
}

grub_err_t chainload_uefi_winpe(const char* new_bootfilename, const bara_efi_ip_address_t* const server_ip, const grub_efi_boolean_t always_use_secureboot_chainload)
{
	grub_errno = GRUB_ERR_NONE;
