...
grub_err_t genericserverrequest(const bara_efi_ip_address_t* server_ip, grub_efi_mac_t* client_mac, const char* const action);
grub_err_t genericbmsfilerequest(const bara_efi_ip_address_t* server_ip, grub_efi_mac_t* client_mac, const char* const action, char** buffer);
grub_err_t generatefilename(char* filename, grub_efi_mac_t* client_mac, const char* action);

/*******************************************************************************************************************/

grub_err_t updateclientbootenv(const bara_efi_ip_address_t* const serverip, grub_efi_mac_t* client_mac, const char* const bootEnvGUID)
{
...
grub_err_t setpathprefix(const bara_efi_ip_address_t* const serverip, grub_efi_mac_t* client_mac, const char* const bootEnvGUID, const char* const pathPrefix)
{
	grub_errno = GRUB_ERR_NONE;
...
grub_err_t generatefilename(char* filename, grub_efi_mac_t* client_mac, const char* action)
{
	...
	else
	{
		...
		else
		{
			...
			{
				grub_snprintf(filename, FILENAME_MAX, "/bbl/%s/%s", mac_address, action);
...
grub_err_t genericserverrequest(const bara_efi_ip_address_t* server_ip, grub_efi_mac_t* client_mac, const char* const action)
{
	...
	else
	{
		...
		if(GRUB_ERR_NONE == generatefilename(filename, client_mac, action))
		{
			grub_uint64_t filesize = 0;
			grub_efi_status_t status = tftpGetFileSize(server_ip, filename, &filesize);
			if(GRUB_EFI_SUCCESS != status)
			{
				grub_error(GRUB_ERR_FILE_READ_ERROR, "Cannot get size of requested file [%s]: error code = %d", filename, status);
			}
			else
			{
				...
				else
				{
					...
					if(GRUB_EFI_SUCCESS != status)
					{
						grub_error(GRUB_ERR_FILE_READ_ERROR, "Cannot read requested file [%s]: error code = %d", filename, status);
					}
					else
					{
						if (0 != grub_strcasecmp(buffer, "ok"))
						{
							grub_error(GRUB_ERR_NET_INVALID_RESPONSE, "Requested file [%s] responses an error: %s!", buffer);
						}
					}
...
grub_err_t loaddynamicbootmenu(bara_efi_ip_address_t* server_ip, grub_efi_mac_t* client_mac, const grub_uint16_t arch_type, char** buffer)
{
	grub_errno = GRUB_ERR_NONE;
